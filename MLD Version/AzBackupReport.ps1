...
# Get date / time and subrakt the LookBackHours 
$since = [datetime]::UtcNow.AddHours(-$LookbackHours).ToString('yyyy-MM-ddTHH:mm:ssZ')
#Get all active Azure subscriptions
$subs = @(Get-AzSubscription | Where-Object State -eq 'Enabled' | ForEach-Object Id)

#KQL query all Revovery Vaults and filter jobs that are with in the LookBackHours
...
# Store job data from KQL query
$jobs = @($(if ($result.PSObject.Properties['Data']) { $result.Data } else { $result }))

# Store all job filtered on their status
